...
def load_dataset(dataset_name: str, raw_dir: str | Path, sample_size: int | None = None, random_state: int = 42) -> tuple[pd.DataFrame, pd.Series, dict[str, Any]]:
    ...
    if sample_size and sample_size < len(df):
        ...
        sampled_parts = []
        for _, group in working.groupby("target", group_keys=False):
            target_count = max(1, round(sample_size * len(group) / len(working)))
            sampled_parts.append(group.sample(n=min(len(group), target_count), random_state=random_state))
        working = pd.concat(sampled_parts, ignore_index=True)
